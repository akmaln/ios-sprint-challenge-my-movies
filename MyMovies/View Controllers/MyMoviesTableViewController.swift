...
class MyMoviesTableViewController: UITableViewController {
    ...
    lazy var fetchedResultsController: NSFetchedResultsController<Movie> = {
        ...
        do {
            ...
            NSLog("error performing initial fetch inside fetchedResulController: \(error)")
        }
        return frc
    }()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: MovieTableViewCell.reuseIdentifier, for: indexPath) as? MovieTableViewCell else {
            fatalError("cant dequeue cell of type \(MovieTableViewCell.reuseIdentifier)")
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "AddMovieModalSegue" {
            if let navC = segue.destination as? UINavigationController,
                let addMovieVC = navC.viewControllers.first as? MovieSearchTableViewController {
